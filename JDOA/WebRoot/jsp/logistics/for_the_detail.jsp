...
<script type="text/javascript">
      ...
      function loadGrid(){
	          ...
	          var coM=[ 	  
                     	  	  {header:'办公用品名称', name : 'fname', align : 'center', width : '145'},
                     		  {header:'入库数量', name : 'finventory_quantity',width: 145,align : 'center'},
                     		  {header:'办公用品型号',name:'ftype',width: 145,align : 'center'},
                     		  {header:'入库时间',name:'finbound_date',width : 145,align : 'center'},
                     		  {header:'办公用品管理员',name:'fstockman',width : 145,align : 'center'},
                     		  {header:'出库数量',name:'fthe_delivery',width : 145,align : 'center'},
                     		  {header:'出库日期',name:'foutbound_date',width : 145,align : 'center'},
                     		  {header:'备注',name:'fcomment',width : 145,align : 'center'},
            ...
            $('#grid').omGrid({
                ...
                colModel : coM,
                autoFit:true,
				dataSource :"queryAllForTheDetail",
            ...
            //保存办公用品库存新增
             ...
             $( "#dialog-modal").omDialog({
                ...
                if(conditions == ""&&requisitioner == ""){ //没要有查询条件，要显示全部数据
                    $('#grid').omGrid("setData", 'queryAllForTheDetail');
                    $('#requisitioner').val('');
                    $('#conditions').val('');
                }else{ //有查询条件，显示查询数据
                    $('#grid').omGrid("setData", 'fuzzyQueryForTheDetail?conditions='+ conditions +'&requisitioner='+requisitioner);
                    $('#requisitioner').val('');
                    $('#conditions').val('');
                  }
			});
      ...
      function editInventory(){
        ...
        $("#edftype").val(data.ftype).attr("disabled","disabled");
        
        $("#edfinventory_quantity").val(data.finventory_quantity).attr("disabled","disabled");
        
        $("#edfcomment").val(data.fcomment); 

        $("#edfthe_delivery").val(data.fthe_delivery).attr("disabled","disabled");
    ...
    function delInventory(){
      ...
      if(index.length<=0){
       ...
       $.ajax({  
		         type: "POST",  
		         url: "deleteForTheDetail?fid="+data.fid, 
		         dataType: "json",  
		         success: function(data){  
		            if(data.success == "success"){ 
		                loadGrid();
...
<body style="padding: 0px;margin: 0px;">
       <div id="fcommonquery">
                                 ...
                                 办公用品管理员:<input id="requisitioner"/> 
       ...
       <div>
    	<table id="grid"></table>
         ...
         <div id="dialog-modal" title="新增出入库明细">
        	<div style="text-align: center;color: red;"><h1>新增出入库明细</h1></div>
         ...
         <form  id="dataFrom">
        	<table width="100%" style="text-align: left;margin-bottom: 20px;">
        	    ...
        	    <td  width="10%" >入库数量:</td>
        	    <td  width="35%" ><input type="text"  id="" name="finventory_quantity" /></td>
        	    </tr>
        	    <tr>
        	   	<td  width="10%" >办公用品型号:</td>
        	    <td  width="35%" ><input type="text"  id="" name="ftype" /></td>
        	    <td  width="10%" >入库日期:</td>
        	    <td  width="35%" ><input type="text"  id="finbound_date" name="finbound_date" onkeypress="return false;" onfocus="SelectDate(this,&quot;yyyy-MM-dd&quot;);" style="background:url(../../image/basic/date.png) no-repeat 130px  -3px;"/></td>
        	    </tr>
        	    <tr>
        	   	<td  width="10%" >出库数量:</td>
        	    <td  width="35%" ><input type="text"  id="" name="fthe_delivery" /></td>
        	    <td  width="10%" >出库日期:</td>
        	    <td  width="35%" ><input type="text" id="foutbound_date" name="foutbound_date" onkeypress="return false;" onfocus="SelectDate(this,&quot;yyyy-MM-dd&quot;);" style="background:url(../../image/basic/date.png) no-repeat 130px  -3px;"/></td>
        	    </tr>
        	    <tr>
	 ...
	 <div id="editInventory" title="修改办公用品出入库明细">
        	<div style="text-align: center;color: red;"><h1>修改办公用品出入库明细</h1></div>
        	<hr style="height:3px;border:none;border-top:3px double red;"/>
				...
				<form id="editfrom">
					<table width="100%" style="text-align: left;margin-top: 30px;">
						...
						<tr>
							<td width="10%">入库数量:</td>
							<td width="35%"><input type="text" id="edfinventory_quantity"
								name="finventory_quantity" /></td>
							
							<td width="10%">出库数量:</td>
							<td width="35%"><input type="text" id="edfthe_delivery"
								name="fthe_delivery" /></td>
						</tr>
						<tr> 
        	    		<td width="10%" >备注:</td>
