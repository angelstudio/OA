	...
	function loadGrid(){
			...
			//------------------------------grid基本信息开始------------------------------------
	         ...
	         var coM=[ 	     {header : '标题', name : 'ftitle', width : 120, align : 'left'}, 
                             ...
                             {header : '调入部门', name : 'findepartment', width : 120, align : 'left'}, 
                             {header : '调出部门', name : 'foutdepartment', width : 120, align : 'left'}, 
                             {header : '使用单位', name : 'fuseunit', width : 120, align : 'left'}, 
                             {header : '申请人', name : 'fapplicant', width : 120, align : 'left'}, 
                             {header : '申请日期', name : 'fapplydate', width : 120, align : 'left'},
                             {header : '调拨类型', name : 'fdbtype', width : 120, align : 'left'},
		    ...
		    $('#grid').omGrid({
                ...
                colModel : coM,
                autoFit:true,
				dataSource :"allotList?ftitle="+bt+"&fuseunit="+fuseunit,
		...
		$("#xqfindepartment").val(data.findepartment).attr("disabled","disabled");
		$("#xqfoutdepartment").val(data.foutdepartment).attr("disabled","disabled");
		$("#xqfassetmanagement").val(data.fassetmanagement).attr("disabled","disabled");
		$("#xqfoutpeople").val(data.foutpeople).attr("disabled","disabled");
		$("#xqfinpeople").val(data.finpeople).attr("disabled","disabled");
		$("#xqfdbtype").val(data.fdbtype).attr("disabled","disabled");
		...
		function loadDetail(fid){
		    $('#showdetailtable').omGrid({
                ...
                colModel : [ {header : '资产序号', name : 'fassetsn', width : 100, align : 'left'},
                             ...
                             {header : '资产现状', name : 'fassetstate', width : 80, align : 'left'}, 
                             {header : '使用年限', name : 'fyearrange', width : 80, align : 'left'}, 
				...
				dataSource : 'AllotDetails?fid='+fid
            });
	}
		...
		function showAddOrEdittable(fid){
		    $('#AddOrEdittable').omGrid({
                ...
                colModel : [ {header : '资产序号', name : 'fassetsn', width : 100, align : 'left',editor:{editable:true}},
                             ...
                             {header : '资产现状', name : 'fassetstate', width : 80, align : 'left',editor:{editable:true}}, 
                             {header : '使用年限', name : 'fyearrange', width : 80, align : 'left',editor:{editable:true}}, 
       ...
       var id = data.fid;
        $.omMessageBox.confirm({
			...
			//将选择的记录的id传递到后台去并执行delete操作
				$.post('deleteAllot',{operation:'delete',fid:id},function(){
				loadGrid();//刷新当前页数据
	...
	$(document).ready(function() {
            	        ...
            	        {label:"修改",
            			 ...
            			 icons : {left : '../../image/common/op-edit.png'},
            	 		 onClick:function(){
                			...
                			isAdd = false;
                			showDialog('修改调拨信息',data);//显示dialog
           ...
           var dialog = $("#dialog-form").omDialog({
            ...
            var showDialog = function(title,rowData){
                ...
                $("#findepartment",dialog).val(rowData.findepartment);
                $("#foutdepartment",dialog).val(rowData.foutdepartment);
                $("#fassetmanagement",dialog).val(rowData.fassetmanagement);
                $("#foutpeople",dialog).val(rowData.foutpeople);
                $("#finpeople",dialog).val(rowData.finpeople);
                $("#fdbtype",dialog).val(rowData.fdbtype);                
            ...
            var submitDialog = function(){
	                    ...
	                    $.post('addOrEditAllot',$("#ipForm").serialize()+"&formData="+formDataStr+"&operation="+operation,function(){
	                    if(isAdd){
...
<body>
	        ...
	        <table>
	         <input name="fid" id="fid" type="hidden"/>
	             <tr>
        	        ...
        	        <td width="10%" >调拨类型:</td>
        		    <td width="35%" ><input type="text" class="sText" id="fdbtype" name="fdbtype"  /></td>
        		    <td width="10%" >调入部门:</td>
        		    <td width="35%"><input type="text" class="sText" id="findepartment" name="findepartment"  /></td>
        	    </tr>
        		<tr>
        		    <td width="10%" >资产类型:</td>
        		    <td width="35%" ><select  id="ftype" name="ftype"  class="sSelect" ><option >请选择</option></select></td>
        		    <td width="10%" >调出部门:</td>
        		    <td width="35%" ><input type="text" class="sText" id="foutdepartment" name="foutdepartment"  /></td>
        		    <td width="10%" >使用单位:</td>
        		    <td width="35%" ><input type="text"class="sText"  id="fuseunit" name="fuseunit"  /></td>
        		</tr>
        		<tr>
        		    <td width="10%" >资产管理部门:</td>
        		    <td width="35%" ><input type="text" class="sText" id="fassetmanagement" name="fassetmanagement"  /></td>
        		    <td width="10%" >调出部门使用人:</td>
        		    <td width="35%" ><input type="text" class="sText" id="foutpeople" name="foutpeople"  /></td>
        		    <td width="10%" >调入部门使用人:</td>
        		    <td width="35%" ><input type="text" class="sText" id="finpeople" name="finpeople"  /></td>
        		</tr>
    ...
    <div id="showdetail" title="资产调拨详情">
        	...
        	<table width="100%" style="text-align: left;" >
        		...
        		<tr><td width="10%" >调入部门:</td>
        		    <td width="35%"><input type="text"  id="xqfindepartment" name="findepartment"  /></td>
        		    <td width="10%" >调出部门:</td>
        		    <td width="35%" ><input type="text"  id="xqfoutdepartment" name="foutdepartment"  /></td>
        		    <td width="10%" >使用单位:</td>
        		    <td width="35%" ><input type="text"  id="xqfuseunit" name="fuseunit"  /></td>
        		</tr>
        		<tr>
        		    <td width="10%" >资产管理部门:</td>
        		    <td width="35%" ><input type="text"  id="xqfassetmanagement" name="fassetmanagement"  /></td>
        		    <td width="10%" >调出部门使用人:</td>
        		    <td width="35%" ><input type="text"  id="xqfoutpeople" name="foutpeople"  /></td>
        		    <td width="10%" >调入部门使用人:</td>
        		    <td width="35%" ><input type="text"  id="xqfinpeople" name="finpeople"  /></td>
        		</tr>
        		<tr>
        		    <td width="10%" >调拨类型:</td>
        		    <td width="35%" ><input type="text"  id="xqfdbtype" name="fdbtype"  /></td>
	        		<td width="10%" >审核人:</td>
	        		<td width="35%" ><input type="text" id="xqfapprover" name="fapprover"  /></td></tr>
	        	    <td width="10%" >审核日期:</td>
	        	    <td width="35%" ><input type="text" id="xqfapproveldate" name="fapproveldate" /></td>  
        		</tr>
        	</table>
