...
<script type="text/javascript">
      ...
      function loadGrid(){
	          ...
	          var coM=[ 	  
                     		  ...
                     		  {header:'领用日期',name:'frecipients_date',width : 145,align : 'center'},
                     		  {header:'办公用品型号',name:'ftype',width : 145,align : 'center'},
                     		  {header:'审核人',name:'fauditor',width : 145,align : 'center'},
                     		  {header:'审核日期',name:'faudit_date',width : 145,align : 'center'},
            ...
            //保存   新增
             ...
             $( "#dialog-modal").omDialog({
                ...
                if(conditions == ""&&requisitioner == ""){ //没要有查询条件，要显示全部数据
                    $('#grid').omGrid("setData", 'queryAllDetails');
                    $('#requisitioner').val('');
                    $('#conditions').val('');
                }else{ //有查询条件，显示查询数据
                    $('#grid').omGrid("setData", 'fuzzyQueryDetail?conditions='+ conditions +'&requisitioner='+requisitioner);
                    $('#requisitioner').val('');
                    $('#conditions').val('');
                  }
			});
	...
	//删除详情
    function delMyRecipientsDetails(){
      ...
      if(index.length<=0){
       ...
       $.ajax({  
		         type: "POST",  
		         url: "deleteMyRecipientsDetail?fid="+data.fid, 
		         dataType: "json",  
		         success: function(data){  
		            if(data.success == "success"){ 
		                loadGrid();
...
<body style="padding: 0px;margin: 0px;">
       ...
       <div>
    	<table id="grid"></table>
         ...
         <div id="dialog-modal" title="新增领用明细">
        	<div style="text-align: center;color: red;"><h1>新增领用明细</h1></div>
         ...
         <form  id="dataFrom">
        	<table width="100%" style="text-align: left;margin-bottom: 20px;">
        	    ...
        	    <td  width="10%" >审核日期:</td>
        	    <td  width="35%" ><input type="text"  id="faudit_date" name="faudit_date" onkeypress="return false;" onfocus="SelectDate(this,&quot;yyyy-MM-dd&quot;);" style="background:url(../../image/basic/date.png) no-repeat 130px  -3px;"/></td>
        	    </tr>
        	    <tr>
        		<td width="10%">审核人:</td>
        		...
        		<td  width="10%" >领用日期:</td>
        	    <td  width="35%" ><input type="text"  id="frecipients_date" name="frecipients_date" onkeypress="return false;" onfocus="SelectDate(this,&quot;yyyy-MM-dd&quot;);" style="background:url(../../image/basic/date.png) no-repeat 130px  -3px;"/></td>
