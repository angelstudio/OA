...
<script type="text/javascript">
      ...
      function loadGrid(){
	          ...
	          var coM=[ 	  
                     	  	  {header:'办公用品名称', name : 'fname', align : 'center', width : '210'},
                     		  {header:'入库总数量', name : 'f_warehouse_total_number',width: 210,align : 'center'},
                     		  {header:'出库总数量',name:'foutbound_total_number',width : 210,align : 'center'},
                     		  {header:'办公用品型号',name:'ftype',width : 210,align : 'center'},
                     		  {header:'备注',name:'fcomment',width: 210,align : 'center'},
            ...
            $('#grid').omGrid({
                ...
                colModel : coM,
                autoFit:true,
				dataSource :"queryAllForTheSummary",
            ...
            //保存   新增
             ...
             $( "#dialog-modal").omDialog({
                ...
                if(conditions == ""){ //没要有查询条件，要显示全部数据
                    ...
                    $('#grid').omGrid("setData", 'fuzzyQueryForTheSummary?conditions='+ conditions );
                    $('#conditions').val('');
                  }
			});
    ...
    function delForTheSummary(){
      ...
      if(index.length<=0){
       ...
       $.ajax({  
		         type: "POST",  
		         url: "deleteForTheSummary?fid="+data.fid, 
		         dataType: "json",  
		         success: function(data){  
		            if(data.success == "success"){ 
		                loadGrid();
...
<body style="padding: 0px;margin: 0px;">
       ...
       <div>
    	<table id="grid"></table>
         ...
         <div id="dialog-modal" title="新增出入库汇总">
        	<div style="text-align: center;color: red;"><h1>新增出入库汇总</h1></div>
         ...
         <form  id="dataFrom">
        	<table width="100%" style="text-align: left;margin-bottom: 20px;">
        	    ...
        	    <td  width="10%" >入库总数量:</td>
        	    <td  width="35%" ><input type="text"  id="" name="f_warehouse_total_number" /></td>
        	    </tr>
        	    <tr>
        	    <td width="10%" >出库总数量:</td>
        		<td width="35%" ><input type="text" id="ftype" name="foutbound_total_number" /></td>
