...
<script type="text/javascript">
      ...
      function loadGrid(){
	          ...
	          var coM=[ 	  
                     		  ...
                     		  {header:'数量', name : 'famount',width: 145,align : 'center'},
                     		  {header:'备注',name:'fcomment',width: 145,align : 'center'},
                     		  {header:'办公用品描述',name:'fdescription',width : 145,align : 'center'},
                     		  {header:'办公用品单价',name:'fprice',width : 145,align : 'center'},
                     		  {header:'类别',name:'fcategory',width : 145,align : 'center'},
                     		  {header:'供应商名称',name:'fsupplier',width : 145,align : 'center'},
                     		  {header:'创建者',name:'fcreator',width : 145,align : 'center'},
                     		  {header:'创建时间',name:'fcreation_time',width : 145,align : 'center'},
                     		  {header:'办公用品型号',name:'ftype',width : 145,align : 'center'},
                     		  {header:'审核人',name:'fapprover',width : 145,align : 'center'},
                     		  {header:'创建部门',name:'fdepartment',width : 145,align : 'center'},
            ...
            //保存办公用品库存新增
             ...
             $( "#dialog-modal").omDialog({
                ...
                if(conditions == ""&&requisitioner == ""){ //没要有查询条件，要显示全部数据
                    $('#grid').omGrid("setData", 'queryAllInventory');
                    $('#requisitioner').val('');
                    $('#conditions').val('');
                }else{ //有查询条件，显示查询数据
                    $('#grid').omGrid("setData", 'fuzzyQueryInventory?conditions='+ conditions +'&requisitioner='+requisitioner);
                    $('#requisitioner').val('');
                    $('#conditions').val('');
                  }
			});
    ...
    function delInventory(){
      ...
      if(index.length<=0){
       ...
       $.ajax({  
		         type: "POST",  
		         url: "deleteInventory?fid="+data.fid, 
		         dataType: "json",  
		         success: function(data){  
		            if(data.success == "success"){ 
		                loadGrid();
...
<body style="padding: 0px;margin: 0px;">
       ...
       <div>
    	<table id="grid"></table>
         ...
         <div id="dialog-modal" title="新增库存">
        	<div style="text-align: center;color: red;"><h1>新增库存</h1></div>
         ...
         <form  id="dataFrom">
        	<table width="100%" style="text-align: left;margin-bottom: 20px;">
        	    ...
        	    <td  width="35%" ><input type="text"  id="" name="fdescription" /></td>
        	    <td  width="10%" >办公用品单价:</td>
        	    <td  width="35%" ><input type="text"  id="" name="fprice" /></td>
        	    </tr>
        	    <tr>
        	   	<td  width="10%" >类别:</td>
        	    <td  width="35%" ><input type="text"  id="" name="fcategory" /></td>
        	    <td  width="10%" >供应商名称:</td>
        	    <td  width="35%" ><input type="text"  id="" name="fsupplier" /></td>
        	    </tr>
        	    <tr>
	 ...
	 <div id="editInventory" title="修改办公用品库存">
        	<div style="text-align: center;color: red;"><h1>修改办公用品库存</h1></div>
