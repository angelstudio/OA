	...
	function loadGrid(){
			...
			//------------------------------grid基本信息开始------------------------------------
	         var coM=[ 	     {header : '标题', name : 'ftitle', width : 120, align : 'left'}, 
                             {header : '申请部门', name : 'fdepartment', width : 120, align : 'left'}, 
		    ...
		    $('#grid').omGrid({
                ...
                colModel : coM,
                autoFit:true,
				dataSource :"propertyData?ftitle="+bt+"&fapplicant="+sqr,
		...
		function loadDetail(fid){
		    $('#showdetailtable').omGrid({
				...
				dataSource : 'properDetails?fid='+fid
            });
	}
      ...
      var data = $("#grid").omGrid("getData").rows[index];
        ...
        $.omMessageBox.confirm({
			...
			//将选择的记录的id传递到后台去并执行delete操作
				$.post('propertydel',{operation:'delete',fid:id},function(){
				loadGrid();//刷新当前页数据
	...
	$(document).ready(function() {
           ...
           var dialog = $("#dialog-form").omDialog({
            ...
            var submitDialog = function(){
	                    ...
	                    $.post('addOrEditProperty',$("#ipForm").serialize()+"&formData="+formDataStr+"&operation="+operation,function(data){
...
<body>
       <div id="fcommonquery">
                  申请人:<span ><input id="sqr" style="height:18px;"/></span>
        ...
        <form id="ipForm">
	        <table>
	         <input name="fid" id="fid" type="hidden"/>
	           <tr>
        	        <td  width="10%" >申请人:</td>
        	        <td  width="35%" ><input type="text" class="sText" disabled="disabled" id="fapplicant"  name="fapplicant"   value="${user.person.fname}" /></td>
        	        <td width="10%" >申请部门:</td>
        		    <td width="35%" ><input type="text" class="sText" disabled="disabled" id="fdepartment" name="fdepartment"  value="${user.person.fssbm}"/></td>
        		    <td width="10%" >申请时间:</td>
        		    <td width="35%" ><input type="text" class="sText" disabled="disabled" id="fapplydate" name="fapplydate"  /></td>
        	    </tr>
	            <tr>
        	        <td  width="10%" >标题:</td>
    ...
    <div id="showdetail" title="资产申请详情">
        	...
        	<table width="100%" style="text-align: left;">
        		<tr><td width="10%" >申请部门:</td>
        		    <td width="35%"><input type="text"  id="xqfdepartment" name="fdepartment" class="sText" /></td>
        		    <td width="10%" >资产类型:</td>
        		    <td width="35%" ><input type="text" id="xqftype" name="ftype" /></td>
        		    <td width="10%" >申请日期:</td>
	        	    <td width="35%" ><input type="text" id="xqfapplydate" name="fapplydate" class="sText"/></td>
        		</tr>
        		<tr>
	        		<td width="10%" >使用单位:</td>
	        		<td width="35%" ><input type="text"  id="xqfuseunit" name="fuseunit" class="sText" /></td>
	        		<td width="10%" >流程状态:</td>
	        		<td width="35%" ><input type="text" id="xqfstatus" name="fstatus" class="sText"/></td>
	        		<td width="10%" >申请人:</td>
	        		<td width="35%" ><input type="text" id="xqfapplicant" name="fapplicant" class="sText" /></td>
        		</tr>
        		<tr>
	        		<td width="10%" >审核人:</td>
	        		<td width="35%" ><input type="text" id="xqfapprover" name="fapprover"  class="sText"/></td>
	        		<td width="10%" >审核日期:</td>
	        	    <td width="35%" ><input type="text" id="xqfapproveldate" name="fapproveldate"class="sText" /></td> 
        	    </tr>
        	</table>
