...
public class JpdlParser extends Parser {
	...
	public static final String CURRENT_VERSION_NAMESPACE = "http://jbpm.org/"
			+ CURRENT_VERSION_JBPM + "/jpdl";
	public static final String CURRENT_VERSION_PROCESS_LANGUAGE_ID = "jpdl-"
			+ CURRENT_VERSION_JBPM;

	private static final String[] SCHEMA_RESOURCES = { "jpdl-4.0.xsd",
		...
		for (String activityResource : DEFAULT_BINDING_RESOURCES) {
			...
			if (resourceUrls.hasMoreElements()) {
				while (resourceUrls.hasMoreElements()) {
					...
					log.trace("loading jpdl bindings from resource: "
							+ resourceUrl);
					jpdlBindingsParser
							.createParse()
							.setUrl(resourceUrl)
				...
				log.trace("skipping unavailable jpdl activities resource: "
						+ activityResource);
			}
		}
	}
	...
	protected Enumeration<URL> getResources(String resourceName) {
		...
		try {
			...
			if (!resourceUrls.hasMoreElements()) {
				resourceUrls = JpdlParser.class.getClassLoader().getResources(
						resourceName);
			}
		} catch (Exception e) {
	...
	public Object parseDocumentElement(Element documentElement, Parse parse) {
		...
		try {
			...
			DeploymentImpl deployment = (DeploymentImpl) parse
					.contextMapGet(Parse.CONTEXT_KEY_DEPLOYMENT);
			if (deployment != null) {
				String processLanguageId = deployment
						.getProcessLanguageId(name);
				if (processLanguageId == null) {
					// if it is not available as a deployment property, check if
					// the
					// jpdlparser attribute specifies a specific jpdl version.
					// this is the case for certain compatibility tests in our
					// test suite
					String jpdlParser = XmlUtil.attribute(documentElement,
							"jpdlparser");
					if (jpdlParser != null) {
						processLanguageId = "jpdl-" + jpdlParser;
					} else {
						// if none of the above, check if this is a parser test
						// run for a specific verion
						// specify the jpdltestversion with
						// "mvn -Djpdlparser=jpdl-4.4 clean install"
						// that way, the whole test suite will be use the
						// specified parser
						jpdlParser = System.getProperty("jpdlparser");
						if (jpdlParser != null) {
							processLanguageId = "jpdl-" + jpdlParser;
						} else {
							// if this process has a namespace, then use the
							// namespace
							// to see what jpdl parser version should be used
							String namespaceUri = documentElement
									.getNamespaceURI();
							if (namespaceUri != null) {
								processLanguageId = "jpdl-"
										+ namespaceUri.substring(16, 19);
							} else {
								// if none of the above, just deploy it as the
				...
				parse.contextMapPut(Parse.CONTEXT_KEY_PROCESS_LANGUAGE_ID,
						processLanguageId);
			}
			...
			Integer version = XmlUtil.attributeInteger(documentElement,
					"version", parse);
			if (version != null) {
			...
			for (Element swimlaneElement : swimlaneElements) {
				String swimlaneName = XmlUtil.attribute(swimlaneElement,
						"name", parse);
				if (swimlaneName != null) {
					SwimlaneDefinitionImpl swimlaneDefinition = processDefinition
							.createSwimlaneDefinition(swimlaneName);
					parseAssignmentAttributes(swimlaneElement,
							swimlaneDefinition, parse);
				}
			}
			...
			if (migrationElement != null) {
				MigrationHelper.parseMigrationDescriptor(migrationElement,
	...
	public void parseActivities(Element documentElement, Parse parse,
		...
		for (Element nestedElement : elements) {
			...
			if ("on".equals(tagName) || "timer".equals(tagName)
					|| "swimlane".equals(tagName)
					|| "migrate-instances".equals(tagName)
					|| "description".equals(tagName))
				continue;

			JpdlBinding activityBinding = (JpdlBinding) getBinding(
					nestedElement, CATEGORY_ACTIVITY);
			...
			try {
				...
				Element descriptionElement = XmlUtil.element(nestedElement,
						"description");
				if (descriptionElement != null) {
					String description = XmlUtil
							.getContentText(descriptionElement);
					activity.setDescription(description);
				}

				String continuationText = XmlUtil.attribute(nestedElement,
						"continue");
				...
				Object parseResult = activityBinding.parse(nestedElement,
						parse, this);
				...
				if (stringTokenizer.countTokens() == 4) {
					try {
						...
						int width = Integer.parseInt(stringTokenizer
								.nextToken());
						int height = Integer.parseInt(stringTokenizer
								.nextToken());
						coordinates = new ActivityCoordinatesImpl(x, y, width,
								height);
				...
				if (coordinates != null) {
					...
					parse.addProblem("invalid coordinates g=\"" + g + "\" in "
							+ activity, nestedElement);
				}
			} finally {
				parse.contextStackPop();
			}
	...
	public TimerDefinitionImpl parseTimerDefinition(Element timerElement,
			Parse parse, ScopeElementImpl scopeElement) {
		TimerDefinitionImpl timerDefinition = scopeElement
		...
		if (duedate != null) {
			...
			String dueDateTimeFormatText = (String) EnvironmentImpl
					.getFromCurrent("jbpm.duedatetime.format", false);
			if (dueDateTimeFormatText == null) {
				dueDateTimeFormatText = "HH:mm dd/MM/yyyy";
			}
			SimpleDateFormat dateFormat = new SimpleDateFormat(
					dueDateTimeFormatText);
			try {
			...
			parse.addProblem(
					"either duedate or duedatetime is required in timer",
					timerElement);
		}
	...
	public void parseOnEvents(Element element, Parse parse,
		...
		for (Element onElement : onElements) {
			...
			if (timerElement != null) {
				TimerDefinitionImpl timerDefinitionImpl = parseTimerDefinition(
						timerElement, parse, scopeElement);
	...
	public void parseOnEvent(Element element, Parse parse,
		...
		if (eventName != null) {
			...
			for (Element eventListenerElement : XmlUtil.elements(element)) {
				JpdlBinding eventBinding = (JpdlBinding) getBinding(
						eventListenerElement, CATEGORY_EVENT_LISTENER);
				if (eventBinding != null) {
					EventListenerReference eventListenerReference = null;
					Object parseResult = eventBinding.parse(
							eventListenerElement, parse, this);
					if (parseResult instanceof EventListener) {
						EventListener eventListener = (EventListener) parseResult;
						eventListenerReference = event
								.createEventListenerReference(eventListener);
					} else {
						Descriptor eventListenerDescriptor = (Descriptor) parseResult;
						eventListenerReference = event
								.createEventListenerReference(eventListenerDescriptor);
					}

					Boolean propagationEnabled = XmlUtil.attributeBoolean(
							eventListenerElement, "propagation", parse);
					...
					if (continuationText != null) {
						if (observableElement instanceof ActivityImpl) {
							if (observableElement.getName() == null) {
								parse.addProblem(
										"async continuation on event listener requires activity name",
							...
							if (transition.getSource().getName() == null) {
								parse.addProblem(
										"async continuation on event listener requires name in the transition source activity",
										eventListenerElement);
							}
						}
						if ("async".equals(continuationText)) {
							eventListenerReference
									.setContinuation(Continuation.ASYNCHRONOUS);
						} else if ("exclusive".equals(continuationText)) {
							eventListenerReference
									.setContinuation(Continuation.EXCLUSIVE);
						}
					}
				} else {
					String tagName = eventListenerElement.getLocalName();
					if (!(observableElement instanceof TransitionImpl && ("condition"
							.equals(tagName) || "timer".equals(tagName)))) {
						parse.addProblem("unrecognized event listener: "
								+ tagName, null, ProblemImpl.TYPE_WARNING,
								eventListenerElement);
					}
				}
			}
	...
	public void parseTransitions(Element element, ActivityImpl activity,
			Parse parse) {
		UnresolvedTransitions unresolvedTransitions = parse
				.contextStackFind(UnresolvedTransitions.class);

		List<Element> transitionElements = XmlUtil.elements(element,
				"transition");
		for (Element transitionElement : transitionElements) {
			String transitionName = XmlUtil
					.attribute(transitionElement, "name");

			System.out.println(XmlUtil
					.attribute(transitionElement, "condition"));
			Element timerElement = XmlUtil.element(transitionElement, "timer");
			if (timerElement != null) {
				TimerDefinitionImpl timerDefinitionImpl = parseTimerDefinition(
						timerElement, parse, activity);
			...
			String id = XmlUtil.attribute(transitionElement, "id");
			transition.setU_id(id);
	...
	public void parseAssignmentAttributes(Element element,
		...
		if (descriptionElement != null) {
			...
			Expression descriptionExpression = Expression.create(
					descriptionText, Expression.LANGUAGE_UEL_VALUE);
		...
		if (assignmentHandlerElement != null) {
			UserCodeReference assignmentHandlerReference = parseUserCodeReference(
					assignmentHandlerElement, parse);
			assignableDefinition
					.setAssignmentHandlerReference(assignmentHandlerReference);
		}
		...
		if (assigneeExpressionText != null) {
			String assigneeExpressionLanguage = XmlUtil.attribute(element,
					"assignee-lang");
			Expression assigneeExpression = Expression.create(
					assigneeExpressionText, assigneeExpressionLanguage);
		...
		if (candidateUsersExpressionText != null) {
			String candidateUsersExpressionLanguage = XmlUtil.attribute(
					element, "candidate-users-lang");
			Expression candidateUsersExpression = Expression.create(
					candidateUsersExpressionText,
					candidateUsersExpressionLanguage);
			assignableDefinition
					.setCandidateUsersExpression(candidateUsersExpression);
		}

		String candidateGroupsExpressionText = XmlUtil.attribute(element,
				"candidate-groups");
		if (candidateGroupsExpressionText != null) {
			String candidateGroupsExpressionLanguage = XmlUtil.attribute(
					element, "candidate-groups-lang");
			Expression candidateGroupsExpression = Expression.create(
					candidateGroupsExpressionText,
					candidateGroupsExpressionLanguage);
			assignableDefinition
					.setCandidateGroupsExpression(candidateGroupsExpression);
		}
	}
	...
	public TaskDefinitionImpl parseTaskDefinition(Element element, Parse parse,
		...
		ProcessDefinitionImpl processDefinition = parse
				.contextStackFind(ProcessDefinitionImpl.class);
		...
		if (swimlaneName != null) {
			JpdlProcessDefinition jpdlProcessDefinition = parse
					.contextStackFind(JpdlProcessDefinition.class);
			SwimlaneDefinitionImpl swimlaneDefinition = jpdlProcessDefinition
					.getSwimlaneDefinition(swimlaneName);
		...
		if (notificationElement != null) {
			parseMailEvent(notificationElement, parse, scopeElement,
					Event.ASSIGN);
		}
		...
		if (reminderElement != null) {
			...
			TimerDefinitionImpl timerDefinition = parseTimerDefinition(
					reminderElement, parse, scopeElement);
	...
	public void parseVariableDefinitions(Element element, Parse parse,
		...
		for (Element variableElement : XmlUtil.elements(element, "variable")) {
			VariableDefinitionImpl variableDefinition = scopeElement
			...
			Boolean isHistoryEnabled = XmlUtil.attributeBoolean(
					variableElement, "history", parse);
			...
			String initExprType = XmlUtil.attribute(variableElement,
					"init-expr-type");
			if (initExpr != null) {
				Expression initExpression = Expression.create(initExpr,
						initExprType);
			...
			if (initDescriptorElement != null) {
				Descriptor initValueDescriptor = (Descriptor) WireParser
						.getInstance().parseElement(initDescriptorElement,
								parse);
			...
			if (sources > 1) {
				parse.addProblem(
						"init attribute and init element are mutually exclusive on element variable",
						variableElement);
			}
	...
	public void parseMailEvent(Element element, Parse parse,
		...
		EventListenerReference eventListenerRef = event
				.createEventListenerReference(eventListener);
		...
		UserCodeReference mailProducer = parseMailProducer(element, parse,
				mailTemplateName);
	...
	public UserCodeReference parseMailProducer(Element element, Parse parse,
		...
		ObjectDescriptor descriptor = new ObjectDescriptor(
				MailProducerImpl.class);
		descriptor.addPropertyInjection("template",
				new ProvidedObjectDescriptor(mailTemplate));
	...
	private MailTemplate parseMailTemplate(Element element, Parse parse,
		...
		if (element.hasAttribute("template")) {
			...
			return findMailTemplate(element, parse,
		...
		parse.addProblem(
				"mail template must be referenced in the 'template' attribute "
						+ "or specified inline", element);
		return null;
	}

	private MailTemplate findMailTemplate(Element element, Parse parse,
			String templateName) {
	...
	public UserCodeReference parseUserCodeReference(Element element, Parse parse) {
		...
		ObjectDescriptor objectDescriptor = parseObjectDescriptor(element,
				parse);
	...
	public Descriptor parseDescriptor(Element element, Parse parse) {
		return (Descriptor) WireParser.getInstance().parseElement(element,
				parse);
	}
