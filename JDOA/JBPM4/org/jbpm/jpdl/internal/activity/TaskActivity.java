...
public class TaskActivity extends JpdlExternalActivity {
	...
	public void execute(ExecutionImpl execution) {
		...
		// 自定义节点
		task.setId(taskDefinition.getId());
	...
	public void signal(ExecutionImpl execution, String signalName,
		...
		if (outgoingTransitions != null && !outgoingTransitions.isEmpty()) {
			// 如果 transition description不为空 则携带了transitionid 根据ID找到数据库配置条件
			// 解析条件满足情况

			//transition = JBPMTransitionUtil.getTransition(outgoingTransitions,
			//		execution.getKey(), execution);
			boolean noOutcomeSpecified = false;
			if (transition == null) {
				noOutcomeSpecified = TaskConstants.NO_TASK_OUTCOME_SPECIFIED
						.equals(signalName);
				if (noOutcomeSpecified
					...
					// specifically
					// used, then the else clause will be used (but the result
					// is
					// the same)
			...
			if (transition != null) {
				...
				ActivityImpl ac = (ActivityImpl) transition.getDestination();
				if (ac.getActivityBehaviour() instanceof TaskActivity) {
					TaskActivity ta = (TaskActivity) ac.getActivityBehaviour();
					String fid = execution.getKey();
					String ftaskId = ta.getTaskDefinition().getId();
					String fexecuteId = execution.getId();
					Long fdbid = execution.getDbid();
					//JBPMTransitionUtil jbpmUtil = new JBPMTransitionUtil();
					//jbpmUtil.createVariableMessage(fid, ftaskId, fexecuteId,
					//		fdbid);
				}
			}
		}
